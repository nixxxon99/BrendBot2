...
async def pick_category(m: Message):
    if m.from_user.id in AI_USERS:
        return
    mapping = {"🥃 Виски": "Виски", "🧊 Водка": "Водка", "🍺 Пиво": "Пиво", "🍷 Вино": "Вино", "🦌 Ягермейстер": "Ликёр"}
    cat = mapping.get(m.text, "")
    names = by_category(cat)
    if not names:
        await m.answer("Пока пусто. Выбери другую категорию.", reply_markup=categories_kb()); return
    from aiogram.utils.keyboard import ReplyKeyboardBuilder
    from aiogram.types import KeyboardButton
    kb = ReplyKeyboardBuilder()
    for n in names: kb.add(KeyboardButton(text=n))
    kb.add(KeyboardButton(text="Назад")); kb.adjust(2)
...
async def send_brand_card(m: Message):
    ...
    item = get_brand(name)
    if not item:
        await m.answer("Не нашёл бренд. Попробуй ещё раз."); return
...
async def suggest(m: Message):
    ...
    from aiogram.types import KeyboardButton
    kb = ReplyKeyboardBuilder()
    for name, _ in suggestions: kb.add(KeyboardButton(text=name))
    kb.add(KeyboardButton(text="Назад")); kb.adjust(1)
