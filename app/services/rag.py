# app/services/rag.py
"""
Локальный семантический индексатор без OpenAI:
- Пытается использовать Sentence Transformers (если установлен пакет sentence-transformers)
- Иначе — строит TF‑IDF индекс через scikit-learn
Индекс сохраняется в data/, чтобы быстро искать без пересборки.
"""
...
try:
    ...
    import numpy as _np  # всё равно пригодится
    _HAS_SBERT = False
...
def rebuild_index(prefer_sbert: bool | None = None) -> str:
    """
    Явная пересборка индекса. Возвращает строку с описанием собранного индекса.
    """
    docs = _load_docs()
    ...
    if use_sbert:
        try:
            ...
            # чистим tfidf, чтобы не путаться
...
def ensure_index():
    if _have_sbert_index() or _have_tfidf_index():
        return
    rebuild_index()

def search_semantic(query: str, top_k: int = 5) -> List[Tuple[float, Dict[str, Any]]]:
    """
    Ищет по индексу и возвращает [(score 0..1, doc_dict), ...]
    - Если есть SBERT — косинус по эмбеддингам.
    - Иначе — косинус по TF‑IDF.
    """
    ...
    if _have_sbert_index():
        try:
            ...
            sims = (emb @ q.T).squeeze()  # так как нормализовано, это косинус
            order = sims.argsort()[::-1]
            out = []
            for idx in order[:top_k]:
                sc = float(sims[idx])
                out.append((sc, docs[idx]))
            return out
        except Exception:
            pass
