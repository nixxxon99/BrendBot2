...
@router.message(F.text == "📦 POSM списание")
...
async def posm_cancel(m: Message):
    ...
    await m.answer("Ок, отменил. Возвращайся в меню.", reply_markup=None)
...
async def posm_flow(m: Message):
    ...
    if step == 1:
        state["item"] = m.text.strip()
        state["step"] = 2
        await m.answer("Сколько штук?")
        return
    if step == 2:
        state["qty"] = m.text.strip()
        state["step"] = 3
        await m.answer("Название и адрес заведения?")
        return
    if step == 3:
        state["venue"] = m.text.strip()
        state["step"] = 4
        await m.answer("Кто отдаёт? (ФИО)")
        return
    if step == 4:
        state["giver"] = m.text.strip()
        state["step"] = 5
        await m.answer("Кто принял? (ФИО)")
        return
    if step == 5:
        ...
        with open(POSM_FILE, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            if first:
                w.writerow(["Дата", "Материал", "Шт", "Заведение", "Отправитель", "Получатель"])
...
async def posm_export(m: Message):
    if not os.path.exists(POSM_FILE):
        await m.answer("Ещё нет записей.")
        return
